...
export function getNavigatorGroupByForLayout(
  ...
  if (layout === 'grid') {
    return resolveGroupBy(pathPreference?.gridGroupBy, navigator.gridGroupBy);
  }

  return resolveGroupBy(pathPreference?.listGroupBy, navigator.listGroupBy);
}
...
export function shouldIncludeItemCountsForSort(
  navigator: UserSettingsNavigator,
  path?: string | null,
): boolean {
  return getNavigatorSortSettingsForLayout(navigator, 'list', path).column === 'items'
    || getNavigatorSortSettingsForLayout(navigator, 'grid', path).column === 'items';
}
